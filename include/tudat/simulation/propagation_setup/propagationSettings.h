...
GravityDeformationPropagatorSettings(
        ...
        std::vector< std::shared_ptr< SingleDependentVariableSaveSettings > >( ),
        const double statePrintInterval = TUDAT_NAN ):
    SingleArcPropagatorSettings< StateScalarType, TimeType >( gravity_deformation_state, initialBodyGravity, integratorSettings->initialTimeDeprecated_, integratorSettings, terminationSettings,
                                                                dependentVariablesToSave ),
    bodiesWithGravityToPropagate_( bodiesWithGravityToPropagate )
{
    for( std::map< std::string, std::shared_ptr< basic_astrodynamics::GravityDeformationModel > >::const_iterator
            deformationModelIt = gravityDeformationModels.begin( ); deformationModelIt != gravityDeformationModels.end( ); deformationModelIt++ )
    {
...
GravityDeformationPropagatorSettings(
        ...
        std::make_shared< SingleArcPropagatorProcessingSettings >( ) ):
    SingleArcPropagatorSettings< StateScalarType, TimeType >(
        gravity_deformation_state, initialBodyGravity, initialTime, integratorSettings, terminationSettings,
        dependentVariablesToSave, outputSettings ),
    bodiesWithGravityToPropagate_( bodiesWithGravityToPropagate ), gravityDeformationModels_( gravityDeformationModels )
{ 
    verifyInput( ); 
...
void resetIntegratedStateModels(
        ...
        const basic_astrodynamics::AccelerationMap& accelerationMap*/ )
{
    gravityDeformationModels_ = simulation_setup::createGravityDeformationModelsMap( bodies, gravityDeformationSettingsMap_ /*, accelerationMap*/ );
}
        ...
        const std::shared_ptr< SingleArcPropagatorSettings< StateScalarType, TimeType > > propagatorSettings )
{
    ...
    case gravity_deformation_state:
    {
        ...
        {
            integratedBodies.push_back( std::make_tuple(
                                            gravityPropagatorSettings->bodiesWithGravityToPropagate_.at( i ), "", PropagatorType( ) ) );
        }
