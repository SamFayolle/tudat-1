            ...
            for ( auto body : integratedBodies )
            {
                std::map< std::string, std::vector< std::shared_ptr< basic_astrodynamics::TorqueModel > > > torqueModels = 
                    ...
                    for ( auto it : torqueModels )
                    {
                        ...
                        {
                            basic_astrodynamics::AvailableTorque torqueType = 
                                basic_astrodynamics::getTorqueModelType( singleTorqueModel );
                            ...
                            for ( auto dependency : dependencies )
                            {
                                ...
                                if ( std::count( listIntegratedStatesPerBody.at( body ).begin( ), 
                                    listIntegratedStatesPerBody.at( body ).end( ), stateType ) )
                                {
                                    // Found torque dependency
                                    updateSettings[ dependency ].push_back( body );
                                    updateModelFunctions.push_back( 
                                        std::bind( &basic_astrodynamics::TorqueModel::updateMembers, singleTorqueModel, std::placeholders::_1 ) );
                ...
                {            
                    ...
                    for ( auto dependency : dependencies )
                    {
                            ...
                            listIntegratedStatesPerBody.at( body ).end( ), stateType ) )
                        {
                            ...
                            updateSettings[ dependency ].push_back( body );
                            updateModelFunctions.push_back( 
                                std::bind( &basic_astrodynamics::GravityDeformationModel::updateMembers, singleDeformationModel, std::placeholders::_1 ) );
                            if ( !updateCurrentModel )
                            {
...
createStateDerivativeUpdaterForDynamicalEquations(
        const std::shared_ptr< SingleArcPropagatorSettings< StateScalarType, TimeType > > propagatorSettings,
        ...
        const simulation_setup::SystemOfBodies& bodies )
{
        ...
        checkTorqueDependencies< StateScalarType, TimeType >( stateDerivativeModelsMap, stateDerivativeModelsToUpdate, 
             listIntegratedStatesPerBody, updateSettings, updateModelFunctions );
        checkGravityDeformationDependencies< StateScalarType, TimeType >( stateDerivativeModelsMap, stateDerivativeModelsToUpdate,
             listIntegratedStatesPerBody, updateSettings, updateModelFunctions );
        ...
        for ( auto it : updateSettings )
        {
                switch( it.first )
                {
                        ...
                        case rotation_rate_derivative_dependency:
                        {
                                for ( auto currentBody : it.second )
                                {
                                        environmentUpdateFunctions[ rotation_rate_derivative_dependency ].push_back(
                                        std::make_pair( currentBody, 
