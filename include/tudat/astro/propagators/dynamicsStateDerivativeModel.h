...
public:
    ...
    DynamicsStateDerivativeModel(
        ...
        stateDerivativeUpdater_( stateDerivativeUpdater )
    {
        ...
        std::map< StateDerivativeDependency, std::vector< std::pair< std::string, 
                ...
                stateDerivativeUpdater->getEnvironmentUpdateFunctions( );
        ...
        for ( auto dependencyTypeIt : environmentUpdateFunctions )
        {
            ...
            for ( unsigned int k = 0 ; k < dependencyTypeIt.second.size( ) ; k++ )
            {
                ...
                for ( unsigned int indexCurrentModel = 0 ; indexCurrentModel < currentStateDerivativeModels.size( ) ; 
                    indexCurrentModel++ )
                {
                    ...
                    {
                        unsigned int indexBody = find( integratedBodies.begin( ), integratedBodies.end( ), bodyToCheck ) 
                        ...
                        std::cout << "indexSingleStateBody " << indexSingleStateBody.first << " " << indexSingleStateBody.second << std::endl;
...

{
    ...
    {
        std::string errorMessage = "Error when getting rotational dynamics model for " +
                bodyUndergoingTorque + " no rotational dynamics models found";
        throw std::runtime_error( errorMessage );
    }
    return modelForBody;
}
