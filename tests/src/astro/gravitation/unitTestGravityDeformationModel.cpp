...
//     std::shared_ptr< KeplerEphemerisSettings > keplerEphemerisSettings = std::make_shared< KeplerEphemerisSettings >( initialKeplerianState, 0.0, effectiveMu, "Jupiter", "J2000" );
...
// -----------------------------------------------------------
   ...
   // Load spice kernels.
    ...
    for ( auto it : results )
    {
        std::cout << it.first << " " << it.second.segment( 0, 3 ).norm( ) << " " << it.second.segment( stateSize - 3, 3 ).transpose( ) << std::endl;
    }
